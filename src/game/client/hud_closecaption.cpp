...
void OnCaptionLanguageChanged(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	Q_snprintf(fn2, sizeof(fn2), "resource/open_fortress_%s.txt", var.GetString());

	// Re-adding the file, even if it's "english" will overwrite the tokens as needed
	if (!IsX360())
	{
		g_pVGuiLocalize->AddFile("resource/closecaption_%language%.txt", "GAME", true);
		g_pVGuiLocalize->AddFile("resource/open_fortress_%language%.txt", "GAME", true);
	}
	...
	if (Q_strlen(var.GetString()) > 0 && Q_stricmp(var.GetString(), uilanguage))
	{
		if (!IsX360())
		{
			...
			{
				char dbfile[512];
				Q_snprintf(dbfile, sizeof(dbfile), "resource/open_fortress_%s.txt", var.GetString());
			...
			{
				char fallback[512];
				Q_snprintf(fallback, sizeof(fallback), "resource/open_fortress_%s.txt", uilanguage);
