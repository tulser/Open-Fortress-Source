...
void CZombieAttack::SelectVictim( CZombie *actor )
{
	...
	for ( CTFPlayer *pPlayer : players )
	{
		...
		if ( pPlayer->m_Shared.InCondInvis() && !pPlayer->m_Shared.InCond( TF_COND_BURNING ) && !pPlayer->m_Shared.InCond( TF_COND_URINE ) && !pPlayer->m_Shared.InCond( TF_COND_MAD_MILK ) && !pPlayer->m_Shared.InCond( TF_COND_STEALTHED_BLINK ) )
			continue;
