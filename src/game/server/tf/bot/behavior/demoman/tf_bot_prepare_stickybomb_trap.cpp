...
ActionResult<CTFBot> CTFBotPrepareStickybombTrap::OnStart( CTFBot *me, Action<CTFBot> *priorAction )
{
	...
	if ( launcher && launcher->m_iReserveAmmo >= launcher->GetMaxClip1() && launcher->Clip1() < launcher->GetMaxClip1() )
	{
		m_bReload = true;
	}
	else
...
ActionResult<CTFBot> CTFBotPrepareStickybombTrap::Update( CTFBot *me, float dt )
{
	...
	if ( m_bReload )
	{
		int iMaxClip = Max( launcher->ReserveAmmo(), launcher->GetMaxClip1() );
	...
	{
		if ( launcher->GetPipeBombCount() >= 8 || launcher->ReserveAmmo() <= 0 )
		{
...
bool CTFBotPrepareStickybombTrap::IsPossible( CTFBot *actor )
{
	...
	/*if ( !actor->IsPlayerClass( TF_CLASS_DEMOMAN ) ) // OFBOT: Allclass support
	{
		return false;
	}*/
