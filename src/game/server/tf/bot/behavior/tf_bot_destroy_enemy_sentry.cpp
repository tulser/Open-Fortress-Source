...
ActionResult<CTFBot> CTFBotDestroyEnemySentry::OnStart( CTFBot *me, Action<CTFBot> *priorAction )
{
	...
	CTFWeaponBase *pWeapon = (CTFWeaponBase *)me->GetWeapon( 0 );

	m_bGrenadeWeapon = false;
	...
	if ( /*me->IsPlayerClass( TF_CLASS_DEMOMAN )*/ m_bGrenadeWeapon ) // OFBOT: Allclass support
...
{
	CTFWeaponBase *primary = const_cast<CTFBot *>( actor )->GetWeaponInSlot( 0, 0 );
	CTFWeaponBase *secondary = const_cast<CTFBot *>( actor )->GetWeaponInSlot( 1, 0 );

	if ( !primary || !secondary )
		return false;

	if ( primary->ReserveAmmo() <= 0 || secondary->ReserveAmmo() <= 0 )
		return false;

	if ( actor->IsExplosiveProjectileWeapon( primary ) || actor->IsExplosiveProjectileWeapon( secondary ) )
		return true;
...
ActionResult<CTFBot> CTFBotUberAttackEnemySentry::OnResume( CTFBot *me, Action<CTFBot> *priorAction )
{
	m_bGrenadeWeapon = false;
