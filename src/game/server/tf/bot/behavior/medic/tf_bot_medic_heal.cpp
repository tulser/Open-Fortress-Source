...
ActionResult<CTFBot> CTFBotMedicHeal::Update( CTFBot *me, float interval )
{
	...
	if ( m_hPatient->m_Shared.InCond( TF_COND_SELECTED_TO_TELEPORT ) )
	{
		...
		for ( int i=0; i<objVector.Count(); ++i )
		{
			...
			if ( teleporter->GetAltMode() == TELEPORTER_TYPE_ENTRANCE && teleporter->IsReady() )
			{
	...
	if ( medigun )
	{
		...
		if ( IsReadyToDeployUber( medigun ) && CanDeployUber( me, medigun ) )
		{
			...
			*/
				...
				if ( m_hPatient->m_Shared.InCondUber() || m_hPatient->m_Shared.InCond( TF_COND_MEGAHEAL ) )
				{
					useUber = false;
				}
	...
	if ( !IsReadyToDeployUber( medigun ) && !me->m_Shared.InCondUber() && !isActivelyHealing && !bUnknownBool && ( isThreatened || outOfHealRange || isPatientObscured ) )
	{
	...
	if ( me->m_Shared.InCondUber() || IsReadyToDeployUber( medigun ) || isHealTargetBlocked )
	{
...
bool CTFBotMedicHeal::IsGoodUberTarget( CTFPlayer *player ) const
{
	...
	// why should it return false? - ficool2
	return true;
}
