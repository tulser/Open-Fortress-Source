...
ActionResult<CTFBot> CTFBotRetreatToCover::Update( CTFBot *me, float dt )
{
	...
	if ( primary && primary->ReserveAmmo() > 0 && me->IsBarrageAndReloadWeapon( primary ) && primary->Clip1() < primary->GetMaxClip1() )
	{
	...
	for ( int i=0; i<me->m_Shared.GetNumHealers(); ++i )
	{
		...
		if ( pMedic )
		{
			CTFWeaponBase *pWpn = ( CTFWeaponBase *)me->Weapon_OwnsThisID( TF_WEAPON_MEDIGUN );

			if ( pWpn )
			{
				CWeaponMedigun *pMedigun = dynamic_cast <CWeaponMedigun*>( pWpn );

				if ( pMedigun && pMedigun->GetChargeLevel() > 0.9f )
					return Action<CTFBot>::Continue();
			}
		}
	}
