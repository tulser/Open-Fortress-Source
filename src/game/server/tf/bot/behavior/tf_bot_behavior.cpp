...
ConVar tf_bot_taunt_victim_chance( "tf_bot_taunt_victim_chance", "0", FCVAR_NONE );
...
ConVar tf_bot_puppet( "tf_bot_puppet", "0", FCVAR_CHEAT, "If set, newly spawned TFBots will stand still and only follow nb_command from the console." );
...
ActionResult<CTFBot> CTFBotMainAction::OnStart( CTFBot *me, Action<CTFBot> *priorAction )
{
	if ( tf_bot_puppet.GetBool() )
		return Action<CTFBot>::SuspendFor( new CTFBotPuppet, "Functioning as a puppet" );
...
ActionResult<CTFBot> CTFBotMainAction::Update( CTFBot *me, float dt )
{
	...
	if ( me->GetTeamNumber() < TF_TEAM_RED || me->GetTeamNumber() > TF_TEAM_MERCENARY )
...
EventDesiredResult<CTFBot> CTFBotMainAction::OnOtherKilled( CTFBot *me, CBaseCombatCharacter *who, const CTakeDamageInfo &info )
{
	...
	if ( pPlayer )
	{
		...
		if ( me->IsSelf( info.GetAttacker() ) && me->IsPlayerClass( TF_CLASS_SPY ) && !TFGameRules()->IsDMGamemode() )
		{
...
void CTFBotMainAction::Dodge( CTFBot *actor )
{
	...
	if ( actor->GetIntentionInterface()->ShouldHurry( actor ) == ANSWER_YES ||
		 /*actor->IsPlayerClass( TF_CLASS_ENGINEER ) ||*/ // OFBOT: Allclass support
		 actor->m_Shared.InCond( TF_COND_DISGUISED ) ||
		 actor->m_Shared.InCond( TF_COND_DISGUISING ) ||
		 actor->m_Shared.InCondInvis() )
	{
...
void CTFBotMainAction::FireWeaponAtEnemy( CTFBot *actor )
{
	...
	if ( pWeapon == nullptr || pWeapon->IsWeapon( TF_WEAPON_MEDIGUN ) )
		return;
	...
	bool bMinigun = ( pWeapon->IsWeapon( TF_WEAPON_MINIGUN ) || pWeapon->IsWeapon( TF_WEAPON_GATLINGGUN ) || pWeapon->IsWeapon( TFC_WEAPON_ASSAULTCANNON )  );

	if ( /*actor->IsPlayerClass( TF_CLASS_HEAVYWEAPONS )*/ bMinigun && !actor->IsAmmoLow() ) // OFBOT: Allclass support
	{
	...
	if ( threat == nullptr || threat->GetEntity() == nullptr || !threat->IsVisibleRecently() )
	{
		...
		{
			// extinguish a teammate if possible
			if ( actor->ShouldFireCompressionBlastAtFriend() )
			{
				actor->PressAltFireButton();
				return;
			}
	...
	{
		if ( actor->ShouldFireCompressionBlast() && !pWeapon->IsWeapon( TF_WEAPON_LIGHTNING_GUN ) )
		{
			actor->PressAltFireButton();
			return;
		}
...
bool CTFBotMainAction::IsImmediateThreat( const CBaseCombatCharacter *who, const CKnownEntity *threat ) const
{
	...
	if ( pPlayer )
	{
		...
		// if ( pPlayer->IsPlayerClass( TF_CLASS_SNIPER ) ) // OFBOT: Allclass support
		...
		if ( !( TFGameRules()->IsDMGamemode() && !TFGameRules()->IsTeamplay() ) ) //OFBOT: Allclass support, everyone is an immediate threat in DM
		{
			if ( actor->m_iSkill > CTFBot::NORMAL && ( pPlayer->IsPlayerClass( TF_CLASS_MEDIC ) || pPlayer->IsPlayerClass( TF_CLASS_ENGINEER ) ) )
				return true;
		}
		else
		{
