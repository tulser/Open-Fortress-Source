...
QueryResultType CTFBotEngineerBuild::ShouldHurry( const INextBot *me ) const
{
	...
	CBaseObject *pSentry = actor->GetObjectOfType( OBJ_SENTRYGUN, 0 );
	CObjectDispenser *pDispenser = (CObjectDispenser *)actor->GetObjectOfType( OBJ_DISPENSER, 0 );

	if ( pSentry && pDispenser && !pSentry->IsBuilding() && !pDispenser->IsBuilding() )
	{
		if ( actor->GetActiveTFWeapon() && ( actor->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_WRENCH || actor->GetActiveTFWeapon()->GetWeaponID() == TFC_WEAPON_WRENCH ) )
		{
			if ( actor->IsAmmoLow() )
