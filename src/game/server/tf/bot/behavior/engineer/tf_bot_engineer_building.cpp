...
ActionResult<CTFBot> CTFBotEngineerBuilding::Update( CTFBot *me, float dt )
{
	...
	CBaseObject *dispenser = me->GetObjectOfType( OBJ_DISPENSER, 0 );
...
{
	CBaseObject *sentry = actor->GetObjectOfType( OBJ_SENTRYGUN, 0 );
	if ( sentry == nullptr )
		return false;
...
{
	CBaseObject *sentry    = actor->GetObjectOfType( OBJ_SENTRYGUN, 0 );
	CBaseObject *dispenser = actor->GetObjectOfType( OBJ_DISPENSER, 0 );
