...
{
	CBaseObject *pSentry    = me->GetObjectOfType( OBJ_SENTRYGUN, 0 );
	CBaseObject *pDispenser = me->GetObjectOfType( OBJ_DISPENSER, 0 );
...
bool CTFBotEngineerBuilding::CheckIfSentryIsOutOfPosition( CTFBot *me ) const
{
	...
	if ( TFGameRules()->GetGameType() == TF_GAMETYPE_PAYLOAD )
	{
...
void CTFBotEngineerBuilding::UpgradeAndMaintainBuildings( CTFBot *me )
{
	...
	CBaseCombatWeapon *pMelee = me->Weapon_GetSlot( 2 );
	if ( pMelee )
		...
		if ( !pSentry->HasSapper() )
		{
			...
			else if ( pSentry->GetTimeSinceLastInjury() > 1.0 )
			{
				if ( pSentry->GetMaxHealth() <= pSentry->GetHealth() && !pSentry->IsBuilding() )
				{
					...
					else if ( pSentry->GetUpgradeLevel() >= /*pSentry->GetMaxUpgradeLevel()*/ 2 )
					{
