...
ActionResult<CTFBot> CTFBotEngineerMoveToBuild::Update( CTFBot *me, float dt )
{
	...
	if ( TFGameRules()->InGametype( TF_GAMETYPE_CP ) && !TFGameRules()->IsInKothMode() && me->GetTeamNumber() == TF_TEAM_BLUE )
	{
		...
		else
		{
			...
			if ( pEntrance && pArea )
			{
				...
				{
					if ( me->GetVisionInterface()->GetPrimaryKnownThreat( true ) && !me->m_Shared.InCondUber() && ShouldRetreat( me ) == ANSWER_YES )
