...
#ifndef _XBOX
#ifdef USE_NAV_MESH
	// TODO: once TF2's navigation format is reverse engineered, remove this horrendous hack, 
	// as otherwise the game crashes trying to load the "invalid" format
	// for now only load nav on DM and MCTF maps since those are generated by our own mod which has working nav
	if ( !Q_strncmp(STRING(gpGlobals->mapname), "dm_", 3) || !Q_strncmp(STRING(gpGlobals->mapname), "mctf_", 5) )
	{
