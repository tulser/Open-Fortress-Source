...
void CNewGameDialog::StartGame( void )
{
	if (m_ChapterPanels.IsValidIndex(m_iSelectedChapter))
	{
		char mapcommand[512];
		mapcommand[0] = 0;
		Q_snprintf(mapcommand, sizeof(mapcommand), "disconnect;progress_enable;exec %s\n", m_ChapterPanels[m_iSelectedChapter]->GetConfigFile());
		...
		// start map
		DevMsg("running command set:\n%s", mapcommand);
		engine->ClientCmd_Unrestricted(mapcommand);

		OnClose();
		DevMsg("THE START GAME BUTTON WAS HIT AND THE MAP SHOULD BE LOADING NOW\n");
	}
}
